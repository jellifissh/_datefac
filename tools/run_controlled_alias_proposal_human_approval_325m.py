...
from datefac.semantic.controlled_alias_proposal_human_approval_325m import (  # noqa: E402
    DEFAULT_OUTPUT_DIR,
    ...
    REVIEWED_NOT_READY_DECISION,
    build_controlled_alias_proposal_human_approval_325m_prepare,
    build_controlled_alias_proposal_human_approval_325m_validate_reviewed,
    load_controlled_alias_proposal_human_approval_325m_inputs,
    load_controlled_alias_proposal_human_approval_325m_reviewed_inputs,
)
from datefac.semantic.controlled_alias_proposal_human_approval_325m_report import (  # noqa: E402
    controlled_alias_proposal_human_approval_325m_markdown,
    write_excel,
    write_json,
)
...
def _blocked_result(output_dir: Path, code: str) -> Dict[str, Any]:
    ...
    workbook_path = output_dir / "controlled_alias_proposal_human_approval_325m_workbook.xlsx"
    summary_json_path = output_dir / "controlled_alias_proposal_human_approval_325m_summary.json"
    package_json_path = output_dir / "controlled_alias_proposal_human_approval_325m_package.json"
    qa_json_path = output_dir / "controlled_alias_proposal_human_approval_325m_qa.json"
    no_apply_path = output_dir / "controlled_alias_proposal_human_approval_325m_no_apply_proof.json"
    notes_path = output_dir / "controlled_alias_proposal_human_approval_325m_review_notes.md"
    ...
    notes_path.write_text(
        controlled_alias_proposal_human_approval_325m_markdown(summary),
        encoding="utf-8",
    )
    return summary
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run 325M controlled alias proposal human approval package."
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        ...
        default=r"D:\_datefac\output\controlled_alias_proposal_human_approval_325m",
    )
    ...
    if args.mode == "prepare":
        if not dry_run_dir.exists():
            _blocked_result(output_dir, "BLOCKED_MISSING_325L_DRY_RUN_DIR")
            print(
                "controlled_alias_proposal_human_approval_325m_summary_json: "
                f"{output_dir / 'controlled_alias_proposal_human_approval_325m_summary.json'}"
            )
            return 0
        ...
        inputs = load_controlled_alias_proposal_human_approval_325m_inputs(
            dry_run_dir=dry_run_dir
        )
        artifacts = build_controlled_alias_proposal_human_approval_325m_prepare(
        ...
        print(f"controlled_alias_proposal_human_approval_325m_workbook: {workbook_path}")
        print(f"controlled_alias_proposal_human_approval_325m_summary_json: {summary_json_path}")
        print(f"controlled_alias_proposal_human_approval_325m_package_json: {package_json_path}")
        print(f"controlled_alias_proposal_human_approval_325m_qa_json: {qa_json_path}")
        print(f"controlled_alias_proposal_human_approval_325m_no_apply_proof_json: {no_apply_path}")
        print(f"controlled_alias_proposal_human_approval_325m_review_notes_md: {notes_path}")
        for key in [
    ...
    reviewed_workbook = Path(args.reviewed_workbook) if args.reviewed_workbook else (
        approval_package_dir / "controlled_alias_proposal_human_approval_325m_workbook.xlsx"
    )
    if not approval_package_dir.exists():
        _blocked_result(output_dir, "BLOCKED_MISSING_325M_APPROVAL_PACKAGE_DIR")
        print(
            "controlled_alias_proposal_human_approval_325m_reviewed_summary_json: "
            f"{output_dir / 'controlled_alias_proposal_human_approval_325m_reviewed_summary.json'}"
        )
        return 0
    if not reviewed_workbook.exists():
        _blocked_result(output_dir, "BLOCKED_MISSING_325M_REVIEWED_WORKBOOK")
        print(
            "controlled_alias_proposal_human_approval_325m_reviewed_summary_json: "
            f"{output_dir / 'controlled_alias_proposal_human_approval_325m_reviewed_summary.json'}"
        )
    ...
    reviewed_inputs = load_controlled_alias_proposal_human_approval_325m_reviewed_inputs(
        approval_package_dir=approval_package_dir,
        reviewed_workbook=reviewed_workbook,
    )
    artifacts = build_controlled_alias_proposal_human_approval_325m_validate_reviewed(
    ...
    workbook_path = output_dir / "controlled_alias_proposal_human_approval_325m_reviewed_workbook.xlsx"
    summary_json_path = output_dir / "controlled_alias_proposal_human_approval_325m_reviewed_summary.json"
    final_result_path = output_dir / "controlled_alias_proposal_human_approval_325m_reviewed_result.json"
    qa_json_path = output_dir / "controlled_alias_proposal_human_approval_325m_reviewed_qa.json"
    notes_path = output_dir / "controlled_alias_proposal_human_approval_325m_reviewed_notes.md"
    ...
    if summary["qa_fail_count"] > 0:
        summary["decision"] = REVIEWED_NOT_READY_DECISION
    ...
    print(f"controlled_alias_proposal_human_approval_325m_reviewed_workbook: {workbook_path}")
    print(f"controlled_alias_proposal_human_approval_325m_reviewed_summary_json: {summary_json_path}")
    print(f"controlled_alias_proposal_human_approval_325m_reviewed_result_json: {final_result_path}")
    print(f"controlled_alias_proposal_human_approval_325m_reviewed_qa_json: {qa_json_path}")
    print(f"controlled_alias_proposal_human_approval_325m_reviewed_notes_md: {notes_path}")
    for key in [
        "approval_record_count",
