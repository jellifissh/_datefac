...
OPTIONAL_FILES = {
    "02A_manual_year_override": "02A_人工年份修正覆盖表.xlsx",
}
...
def _to_col_key(name: str) -> str:
    return _norm(name).lower().replace(" ", "").replace("_", "").replace("-", "")
...
def _check_files(delivery_dir: Path) -> Tuple[List[Dict[str, object]], Dict[str, Path]]:
    required_paths = {k: delivery_dir / v for k, v in REQUIRED_FILES.items()}
    optional_paths = {k: delivery_dir / v for k, v in OPTIONAL_FILES.items()}
...
def _required_cols(
    ...
    _check(
        ...
        "PASS" if not missing else status_if_missing,
        severity_if_missing if missing else "INFO",
        len(missing),
        "missing=" + "|".join(missing) if missing else "",
        "检查列名或重建该文件。" if missing else "",
    )
...
def _duplicate_keys(
    ...
    if grouped.empty:
        _check(checks, f"{table}_duplicate_keys", "PASS")
    else:
        _check(
            checks,
            f"{table}_duplicate_keys",
            "FAIL" if fail_if_found else "WARN",
            "ERROR" if fail_if_found else "WARN",
            len(grouped),
            f"duplicate_key_groups={len(grouped)}",
            "检查去重逻辑或冲突处理。",
        )
...
def _high_risk_flags(df: pd.DataFrame, table: str, checks: List[Dict[str, object]], details: List[Dict[str, object]]) -> None:
    ...
    _check(
        checks,
        f"{table}_high_risk_flags",
        "PASS" if count == 0 else "FAIL",
        "ERROR" if count else "INFO",
        count,
        f"rows_with_high_risk_flags={count}",
        "高风险 flags 不应进入 01/06。" if count else "",
    )
...
def _test_tokens(
    ...
    for token in TEST_TOKENS:
        if token.lower() in template_text.lower():
            ...
            details.append(
                {
                    "file_key": "06C_template",
                    "row_index": "",
                    "column": "text",
                    "token": token,
                    "cell_value": "template_contains_token",
                    "severity": "WARN",
                }
            )
    ...
    _check(
        ...
        severity,
        hard_hits,
        f"hard_hits={hard_hits}; template_hits={template_hits}",
        "清理 TEST/20266/987654.321，模板建议替换为正式示例。" if hard_hits or template_hits else "",
    )
...
def _manual_queue(
    ...
    _check(
        checks,
        "manual_queue_required_review_columns",
    ...
    for i, r in df02.iterrows():
        ...
        if len(tokens) > 1:
            ...
            issue_details.append(
                {
                    "row_index": int(i),
                    "issue_type": "multi_year",
                    "asset_package": _norm(r.get("asset_package")),
                    "standard_metric": _norm(r.get("standard_metric")),
                    "year": year,
                    "tokens": "|".join(tokens),
                    "how_to_fix": "year 只能填单一年份，如 2026E。",
                }
            )
        ...
        if wants_apply and (not corrected or len(tokens) != 1):
            ...
            issue_details.append(
                {
                    "row_index": int(i),
                    "issue_type": "corrected_row_not_ready",
                    "asset_package": _norm(r.get("asset_package")),
                    "standard_metric": _norm(r.get("standard_metric")),
                    "year": year,
                    "tokens": "|".join(tokens),
                    "how_to_fix": "填写 corrected_value，并确保 year 为单一年份。",
                }
            )
    _check(
        checks,
        "manual_queue_multi_year",
        "PASS" if multi_year == 0 else "WARN",
        "WARN" if multi_year else "INFO",
        multi_year,
        f"multi_year_rows={multi_year}",
    )
    _check(
        checks,
        "manual_queue_corrected_rows_ready",
        "PASS" if not_ready == 0 else "WARN",
        "WARN" if not_ready else "INFO",
        not_ready,
        f"not_ready_rows={not_ready}",
    )
...
def _check_02a(
    ...
    df02a, status = _read_excel(path)
    read_rows.append(
        {
            "file_key": "02A_manual_year_override",
            "read_status": status,
            "rows": len(df02a),
            "cols": len(df02a.columns),
        }
    )

    if status == "missing":
        _check(checks, "02A_presence", "SKIP", "INFO", 0, "02A_missing_optional")
        return pd.DataFrame()
    if not status.startswith("ok"):
        _check(checks, "02A_read", "FAIL", "ERROR", 1, status, "修复 02A 文件可读性。")
        return pd.DataFrame()

    _check(checks, "02A_presence", "PASS", "INFO", 1, "02A_found")
    _required_cols(
        df02a,
    ...
    # 02A duplicate keys
    _duplicate_keys(df02a, "02A_manual_year_override", checks, duplicate_details, fail_if_found=True)

    # year and numeric checks for rows intended to apply
    year_invalid = 0
    numeric_invalid = 0
    for i, r in df02a.iterrows():
        review_status = _norm(r.get("review_status")).lower()
        use_flag = _norm(r.get("use_corrected_value")).lower()
        wants_apply = review_status in STATUS_CORRECTED and use_flag in TRUE_VALUES
        if not wants_apply:
            continue
        ...
        if len(tokens) != 1:
            ...
            issue_details.append(
                {
                    "table": "02A_manual_year_override",
                    "row_index": int(i),
                    "issue_type": "invalid_year_token",
                    "asset_package": _norm(r.get("asset_package")),
                    "standard_metric": _norm(r.get("standard_metric")),
                    "year": year,
                    "tokens": "|".join(tokens),
                    "how_to_fix": "year 需为单一年份，如 2027E。",
                }
            )

        corrected_value = _norm(r.get("corrected_value")).replace(",", "")
        try:
            ...
            issue_details.append(
                {
                    "table": "02A_manual_year_override",
                    "row_index": int(i),
                    "issue_type": "corrected_value_not_numeric",
                    "asset_package": _norm(r.get("asset_package")),
                    "standard_metric": _norm(r.get("standard_metric")),
                    "year": year,
                    "tokens": "|".join(tokens),
                    "how_to_fix": "corrected_value 需为可数值化文本，例如 204.59。",
                }
            )

    _check(
        checks,
        "02A_single_year_for_effective_rows",
        "PASS" if year_invalid == 0 else "WARN",
        "WARN" if year_invalid else "INFO",
        year_invalid,
        f"invalid_rows={year_invalid}",
        "修正 year 为单一年份。",
    )
    _check(
        checks,
        "02A_numeric_corrected_value_for_effective_rows",
        "PASS" if numeric_invalid == 0 else "WARN",
        "WARN" if numeric_invalid else "INFO",
        numeric_invalid,
        f"invalid_rows={numeric_invalid}",
        "修正 corrected_value 为数值。",
    )
    return df02a
...
def check_delivery_state(delivery_dir: Path, write_report: bool = True) -> Dict[str, object]:
    ...
    missing_required_files = sum(
        1 for r in file_rows if r["required"] and not r["exists"]
    )
    _check(
        checks,
        "required_delivery_files",
        "PASS" if missing_required_files == 0 else "FAIL",
        "ERROR" if missing_required_files else "INFO",
        missing_required_files,
        f"missing_required_files={missing_required_files}",
    )
    ...
    _check(
        checks,
        "read_excels",
        "PASS" if not failed_reads else "FAIL",
        "ERROR" if failed_reads else "INFO",
        len(failed_reads),
        f"failed_reads={len(failed_reads)}",
    )
    ...
    _required_cols(
        df06,
        "06_final",
        ["asset_package", "standard_metric", "year", "final_value", "final_value_source"],
        checks,
        col_details,
    )
    ...
    # Optional 02A checks
    df02a = _check_02a(
        paths["02A_manual_year_override"],
        checks,
        read_rows,
        col_details,
        duplicate_details,
    ...
    _test_tokens(
        {
            "01_auto_trusted": df01,
            "02_manual_queue": df02,
            "02A_manual_year_override": df02a,
            "06_final": df06,
            "06A_apply_detail": df06a,
            "06B_unresolved": df06b,
        },
        template_text,
        checks,
        token_details,
