...
from datefac.table_bakeoff.docling_metric_probe import DoclingMetricProbeConfig, run_docling_metric_probe


def main() -> int:
    parser = argparse.ArgumentParser(description="Run 321E2 Docling unified mapping probe.")
    ...
    result = run_docling_metric_probe(
        DoclingMetricProbeConfig(
    ...
    print(f"docling_unified_mapping_excel: {result.get('excel_path', '')}")
    print(f"docling_unified_mapping_summary_json: {result.get('summary_json_path', '')}")
    print(f"docling_unified_mapping_report_md: {result.get('report_md_path', '')}")
    for key in [
        "input_image_count",
        "docling_table_count",
        "unified_table_count",
