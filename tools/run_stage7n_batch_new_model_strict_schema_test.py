...
IN_QUICK_SUMMARY = BASE_DIR / "output" / "stage7n_quick_new_model_smoke_test" / "198_stage7n_quick_new_model_summary.json"

OUT_DIR = BASE_DIR / "output" / "stage7n_batch_new_model_strict_schema_test"
OUT_SUMMARY = OUT_DIR / "199_stage7n_batch_new_model_summary.json"
OUT_REPORT = OUT_DIR / "199_stage7n_batch_new_model_report.md"
OUT_SELECTED = OUT_DIR / "199_stage7n_selected_requests.jsonl"
OUT_RAW = OUT_DIR / "199_stage7n_raw_responses_sanitized.jsonl"
OUT_AUDIT = OUT_DIR / "199_stage7n_validation_audit.xlsx"
OUT_VALID = OUT_DIR / "199_stage7n_validated_suggestions.xlsx"
OUT_REJ = OUT_DIR / "199_stage7n_rejected_suggestions.xlsx"
OUT_COST = OUT_DIR / "199_stage7n_cost_latency_report.json"
...
def _pick_requests() -> Tuple[List[Dict[str, Any]], str]:
    if IN_SLIM_REQ.exists():
        rows = _load_jsonl(IN_SLIM_REQ)
        if len(rows) >= 3:
            return rows[:3], "stage7m_fix_slim_requests"
    rows = _load_jsonl(IN_STAGE7I_REQ)
    rows = sorted(rows, key=lambda x: (len(x.get("candidate_rows", [])), _norm(x.get("manual_review_reason")), _norm(x.get("review_id"))))
    return rows[:3], "stage7i_requests_fallback"
...
def main() -> int:
    ...
    required = [IN_SCHEMA, IN_RULES, IN_QUICK_SUMMARY]
    for p in required:
        if not p.exists():
            _write_json(
                OUT_SUMMARY,
                {
                    "stage": "stage7n_batch_new_model_strict_schema_test",
    ...
    new_model_batch_test_pass = bool(
        len(selected) == 3
        and schema_valid_response_count == 3
        ...
        and validated_suggestion_count == 3
    )
    ...
    recommended_next_step = (
        "stage7o_five_case_new_model_batch_test"
        if new_model_batch_test_pass
        else f"fix_{failure_reason}_and_retry_stage7n_batch"
    )

    summary = {
        "stage": "stage7n_batch_new_model_strict_schema_test",
        "mode": "real_api_three_case_new_model_strict_schema",
        "based_on_stage7n_quick_commit": "0ce198e427ec09a3bb39df85190a997502f97a99",
        ...
        "new_model_batch_test_pass": new_model_batch_test_pass,
        "failure_reason": failure_reason,
        "recommended_next_step": recommended_next_step,
        "quick_smoke_recommended_next_step": _norm(quick_summary.get("recommended_next_step")),
    }
    ...
    report = "\n".join(
        [
            ...
            "",
            "## Decision",
            f"- new_model_batch_test_pass: {summary['new_model_batch_test_pass']}",
