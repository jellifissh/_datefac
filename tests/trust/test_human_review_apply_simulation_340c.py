...
def _build_review_queue() -> pd.DataFrame:
    ...
    for idx in range(1, 78):
        row = {
            ...
            "unit_before": "" if idx in {1, 5} else ("百万元" if idx == 2 else ("%")),
            "source_page": 12,
    ...
    rows[0]["reviewer_corrected_unit"] = "百万元"
    rows[0]["reviewer_notes"] = "补金额单位"

    rows[1]["reviewer_decision"] = "CONFIRM_AS_REVIEWED"
    rows[1]["reviewer_notes"] = "确认"

    rows[2]["reviewer_decision"] = "CONFIRM_AS_REVIEWED"
    rows[2]["reviewer_notes"] = "同比百分比正确"
    ...
    rows[3]["reviewer_corrected_unit"] = "元"
    rows[3]["reviewer_notes"] = "修正单位"
    ...
    rows[4]["reviewer_corrected_unit"] = "百万元"
    rows[4]["reviewer_notes"] = "补金额单位"
...
def test_build_human_review_apply_simulation_340c(tmp_path: Path) -> None:
    ...
    _write_excel(
        ...
        {
            "01_REVIEW_QUEUE": _build_review_queue(),
        },
    )
