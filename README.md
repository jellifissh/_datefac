...
很多 PDF 抽取项目在演示时只回答一个问题：parser 有没有把表格和数字识别出来。DateFac 当前回答的是更偏工程治理的问题：当 parser 已经给出候选结果以后，系统能不能保留 provenance，识别 unit 风险，把记录保守地分成 trusted 与 review_required，把人工判断隔离在 no write-back 的 sidecar 流程里，再把最终状态包装成适合 GitHub、面试、远程协作和项目交接的 demo 文档。当前项目状态是 `DEMO_READY_AFTER_HUMAN_UNIT_REVIEW_PREVIEW`，也就是“完成了人工 unit review 反馈后的 reviewed preview 展示状态”，但仍然 `client_ready = false`，`production_ready = false`。这意味着它适合拿来展示工程思路、review 闭环、preview 刷新和风险边界，不适合被说成正式客户端交付系统，也不适合被说成生产写回已经完成。
...
| 字段 | 当前值 | 说明 |
|---|---|---|
| `project_status` | `DEMO_READY_AFTER_HUMAN_UNIT_REVIEW_PREVIEW` | 当前是人工 unit review 后的 reviewed preview 演示状态 |
...
| `qa_fail_count` | `0` | 当前 release audit QA 通过 |

### 当前 headline metrics

| 指标 | 当前值 | 来源阶段 |
|---|---:|---|
| unfamiliar PDFs | 13 | 330L / 331A / 331B / 332A 叙事基线 |
...
| `reviewed_trusted_preview_row_count` | 98 | 330K4 / 331B / 332A |
| `human_rejected_row_count` | 18 | 330K4 / 331B / 332A |
| `remaining_review_required_after_unit_review_count` | 1 | 330K4 / 331B / 332A |
| `apply_plan_row_count` | 21 | 330K3 / 330K4 / 331B / 332A |
| `overclaim_risk_count` | 0 | 332A |
| `qa_fail_count` | 0 | 332A |

为避免读者只扫表格而忽略真实边界，下面再用精确字符串重申当前 reviewed preview 状态：
...
这些行不是无意义重复，而是为了确保任何引用 README 的人都能直接看到当前状态，不把 reviewed preview 误说成正式交付结果。尤其是 `human_rejected_row_count = 18` 和 `remaining_review_required_after_unit_review_count = 1`，它们提醒读者：系统当前并不是“把所有候选都推入 trusted”，而是明确保留了拒绝和未决状态。
...
下面是当前 330K2 到 332A 的 demo 路线。它们都属于 sidecar / demo / preview / no write-back 流程。
...
1. 仍然是 demo preview，不是正式客户端导出。
2. parser 质量仍然是瓶颈，尤其是复杂表格、噪声行和跨列修复场景。
3. reviewed preview 不是 production export，也没有正式 write-back。
...
1. 做更干净的 client-facing export layer。
...
| Field | Value | Meaning |
|---|---|---|
| `project_status` | `DEMO_READY_AFTER_HUMAN_UNIT_REVIEW_PREVIEW` | The project currently reflects a reviewed preview after manual unit review feedback |
...
| `qa_fail_count` | `0` | The current release-audit QA is clean |
...
| `human_rejected_row_count` | 18 | Rows intentionally kept out of trusted preview after manual review |
| `remaining_review_required_after_unit_review_count` | 1 | Rows still unresolved after the current review cycle |
...
The current documented pipeline begins at the 330K2 packaging stage and ends at the 332A release-audit stage. It assumes cached upstream outputs already exist.
...
1. It is still a reviewed preview demo rather than a production release.
2. Parser quality remains a bottleneck, especially for noisy or structurally messy tables.
3. It still needs a cleaner client-facing export layer.
...
1. build a cleaner client-facing export layer
