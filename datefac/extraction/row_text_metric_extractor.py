...
CASHFLOW_RULES: List[Tuple[str, List[str]]] = [
    ...
    ("other_operating_cf", ["其它", "其他"]),
    ("operating_cash_flow", ["经营活动现金流"]),
    ("capex", ["资本开支"]),
    ("other_investing_cash_flow", ["其它投资现金流", "其他投资现金流"]),
    ("investing_cash_flow", ["投资活动现金流"]),
    ("equity_financing", ["权益性融资"]),
    ("debt_net_change", ["负债净变化"]),
    ("dividend_interest_paid", ["支付股利、利息", "支付股利", "利息"]),
    ("other_financing_cash_flow", ["其它融资现金流", "其他融资现金流"]),
...
def _normalize_value(token: str) -> str:
    t = _norm(token).replace("锛?", "(").replace("锛?", ")")
...
def _metric_match(row_text: str, cashflow_context: bool) -> Tuple[str, str]:
    ...
    if cashflow_context and ("其它投资现金流" in row_text or "其他投资现金流" in row_text):
        return "other_investing_cash_flow", "其它投资现金流"
    if cashflow_context and ("其它融资现金流" in row_text or "其他融资现金流" in row_text):
        return "other_financing_cash_flow", "其它融资现金流"
    if cashflow_context and ("其它" in row_text or "其他" in row_text):
        return "other_operating_cf", "其它"
...
def extract_metric_candidates_from_repaired_rows(
    ...
    for rr in repaired_rows:
        ...
        candidate_row_id = f"{_norm(rr.get('extracted_table_id'))}:{_norm(rr.get('row_index'))}:{metric_code}"
        for i in range(n):
            raw_value = nums[i]
