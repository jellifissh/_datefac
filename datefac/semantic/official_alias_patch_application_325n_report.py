...
def official_alias_patch_application_325n_markdown(summary: Dict[str, Any]) -> str:
    lines: List[str] = [
        "# Official Alias Patch Application 325N",
        "",
        "## Decision",
        ...
        f"- conflicting_existing_alias_count: {summary.get('conflicting_existing_alias_count', 0)}",
        ...
        f"- formal_scope_rules_unchanged_confirmed: {summary.get('formal_scope_rules_unchanged_confirmed', False)}",
        f"- rollback_backup_paths: {summary.get('rollback_backup_paths', {})}",
        "",
        "## Hashes",
        f"- semantic_alias_candidates_hash_before: {summary.get('semantic_alias_candidates_hash_before', '')}",
        f"- semantic_alias_candidates_hash_after: {summary.get('semantic_alias_candidates_hash_after', '')}",
        f"- formal_scope_rules_hash_before: {summary.get('formal_scope_rules_hash_before', '')}",
        f"- formal_scope_rules_hash_after: {summary.get('formal_scope_rules_hash_after', '')}",
        "",
        "## QA",
    ...
    lines.extend(
        [
            "## Notes",
            "- 325N writes only the official semantic alias override asset.",
            "- 325N never writes the formal scope rules asset.",
            "- 325N does not run semantic adjudication, sandbox replay, or post-patch regression.",
            "",
        ]
    )
    return "\n".join(lines)
...
def rollback_instructions_markdown(summary: Dict[str, Any], rollback_rows: List[Dict[str, Any]]) -> str:
    lines: List[str] = [
        "# Official Alias Patch Application 325N Rollback Instructions",
        "",
        "## Rollback Rule",
        "- Restore only the official alias asset from the 325N backup file.",
        "- Do not revert unrelated source files, outputs, temp files, or the formal scope rules asset.",
        "",
        "## Backup Files",
    ]
    ...
    for row in rollback_rows:
        lines.append(
            f"- {row.get('patch_operation_id', '')}: {row.get('rollback_action', '')} on {row.get('target_path', '')}"
        )
