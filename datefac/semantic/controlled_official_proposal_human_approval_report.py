...
def controlled_official_proposal_human_approval_markdown(summary: Dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "## Safety",
        f"- all_decisions_pending_human_approval: {summary.get('all_decisions_pending_human_approval', False)}",
    ...
    lines.extend(
        [
            "## Notes",
            "- 323L prepares a human approval package only.",
            "- 323L does not apply semantic rules and does not modify official assets.",
            "",
        ]
    )
    return "\n".join(lines)
