...
def alias_adjudicator_response_collection_325f_markdown(summary: Dict[str, Any]) -> str:
    lines = [
        "# Alias Adjudicator Response Collection 325F",
        "",
        "## Decision",
    ...
    lines.extend(
        [
            "## Notes",
            "- 325F stores raw/manual responses only.",
            "- 325F does not validate response schema or run deterministic gates.",
            "- 325F does not call an LLM/adjudicator, apply rules, create official candidates, create proposals, or run sandbox replay.",
            "",
        ]
    )
    return "\n".join(lines)
