...
def controlled_official_proposal_dry_run_324j_markdown(summary: Dict[str, Any]) -> str:
    lines: List[str] = [
        "# Controlled Official Proposal Dry Run 324J",
        "",
        "## Decision",
    ...
    lines.extend(
        [
            "## Notes",
            "- 324J generates output-only patch-operation preview artifacts.",
            "- 324J resolves virtual after-state without writing the official scope asset.",
...
def controlled_official_proposal_dry_run_324j_rollback_markdown(
    rollback_plan_rows: List[Dict[str, Any]]
) -> str:
    lines = [
        "# Controlled Official Proposal Dry Run 324J Rollback Plan",
        "",
        "## Rollback Principles",
        "- 324J is output-only and does not modify official assets.",
        "- Any rollback in this stage means removing dry-run preview operations from the 324J package only.",
        "",
