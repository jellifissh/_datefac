...
def controlled_official_proposal_from_323i_report_markdown(summary: Dict[str, Any]) -> str:
    lines = [
        "# Controlled Official Proposal From 323I 323J",
        "",
        "## Decision",
    ...
    lines.extend(
        [
            "## Notes",
            "- 323J creates a controlled proposal package only.",
            "- No official mapping or override asset is modified in this stage.",
            "- Duplicate source suggestions remain provenance only and are not re-expanded into separate proposals.",
            "",
        ]
    )
    return "\n".join(lines)
