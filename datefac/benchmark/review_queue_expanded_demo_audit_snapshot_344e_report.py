...
def report_markdown(summary: Dict[str, Any], qa_json: Dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "## 中文摘要",
        "- 344E 关闭了 29-row expanded trusted demo arc 的最终 review/demo 审计快照与交接层。",
        "- expanded package 合并了 10 条 prior demo rows 和 19 条 source-check resolved rows。",
        "- 其中 10 条 source-check rows 为 confirmed，9 条为 corrected，corrected rows 保持 YOY/% 语义。",
        "- 所有输出仍然只用于 review/demo，formal client export 继续禁止，也没有发生 production write-back。",
        "- 下一步默认建议是 345A formal export readiness gap assessment。",
        "",
        "## English Summary",
        "- 344E closes the 29-row expanded trusted demo arc with final audit snapshot and handoff materials.",
        "- The package combines 10 demo rows and 19 source-check resolved rows.",
        "- Ten source-check rows are confirmed and nine are corrected with YOY/% semantics preserved.",
        "- All outputs remain review/demo only. Formal client export is still blocked.",
        "",
        "## Key Metrics",
        f"- decision: {summary.get('decision', '')}",
        f"- review_queue_schema_version: {summary.get('review_queue_schema_version', '')}",
        f"- input_expanded_export_row_count: {summary.get('input_expanded_export_row_count', 0)}",
        ...
        f"- expanded_demo_arc_closed: {summary.get('expanded_demo_arc_closed', False)}",
        f"- ready_for_345a: {summary.get('ready_for_345a', False)}",
        f"- recommended_345a_scope: {summary.get('recommended_345a_scope', '')}",
