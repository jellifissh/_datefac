...
当前任务是 docs-only contract update，不是代码修复。

重点：

```text
把 output_schema_guardrails 写成当前 Excel audit runner 的标准契约
统一 logical row count 与 physical CSV row count 口径
保留 readiness gates closed / external calls 0 / output 默认不提交
```
...
或 348N-R7 pilot another workbook family under guardrails
```
