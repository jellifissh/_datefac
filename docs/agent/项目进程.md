...
当前任务是 QA/review，不是代码修复。

重点审查：

```text
UNKNOWN_ROW -> NARRATIVE_ASSERTION 是否过度泛化
clean_data 是否仍干净
review_queue 是否仍保留 mixed valuation row
business matrix / AIDC comparative rows 作为 narrative review 是否可接受
是否不应先转 MARKET_REFERENCE_ROW
```
